...
class ViralIntelligence:
    ...
    def _emotion(self, text: str) -> str:
        ...
        if any(word in lowered for word in ("fear", "risk", "collapse", "warning")):
            return "fear"
        if any(word in lowered for word in ("beautiful", "amazing", "incredible", "remarkable", "stunning", "gorgeous")):
            return "awe"
    ...
    def _topic(self, query: str, text: str, hinted_topic: str = "") -> str:
        ...
        if (tokens & SHOPPING_TERMS) and not any(phrase in blob for phrase in MISSION_PHRASES):
            return "off-topic"
        if any(phrase in blob for phrase in ("runway", "couture", "vintage", "high fashion")):
            return "fashion"
        category_map = (
            ("entertainment", ("entertainment", "celebrity", "movie", "music", "gaming", "streaming", "box office")),
            ("memes", ("meme", "memes", "humor", "funny", "viral")),
            ("culture", ("internet", "pop culture", "viral", "meme", "aesthetic", "trendy")),
            ("beauty", ("makeup", "skincare", "vogue", "glamour", "model", "runway")),
            ("fashion", ("couture", "style", "vintage", "designer", "apparel", "runway")),
        )
