...
log = logging.getLogger("poco.llm")
...
ROLE_ENV_MAP = {
    "director": "poco_MODEL_DIRECTOR",
    "trend": "poco_MODEL_TREND",
    "creator": "poco_MODEL_CREATOR",
    "rephrase": "poco_MODEL_REPHRASE",
    "selector": "poco_MODEL_SELECTOR",
    "coding": "poco_MODEL_CODING",
...
class LocalLLM:
    def __init__(self, model: str | None = None):
        ...
        self.enable_local_ollama = _env_enabled("poco_ENABLE_LOCAL_OLLAMA", "1")
        self.enable_ollama_cli_fallback = _env_enabled("poco_ENABLE_OLLAMA_CLI_FALLBACK", "0")
        self.preference = os.environ.get("poco_LLM_PREFERENCE", "remote-first").strip().lower() or "remote-first"
        ...
        primary = (model or os.environ.get("poco_PRIMARY_MODEL", "qwen2.5:0.5b")).strip()
        ...
        payload = json.dumps(
            {
                ...
                "prompt": prompt,
                "stream": False,
                "keep_alive": os.environ.get("poco_OLLAMA_KEEP_ALIVE", "20m"),
                "options": {
                    "num_predict": _env_int("poco_OLLAMA_NUM_PREDICT", 180),
                    "temperature": _env_float("poco_OLLAMA_TEMPERATURE", 0.55),
                },
            }
        ).encode("utf-8")
    ...
    def _candidate_order(self, candidates: list[str], backend: str) -> list[str]:
        ...
        if _env_enabled("poco_SKIP_OLLAMA_MODEL_DISCOVERY", "0"):
            return usable
    ...
    def ask(self, prompt: str, timeout: int = 45, role: str = "director") -> str:
        ...
        if not self.enable_local_ollama and not self._logged_local_disabled:
            log.info("Local Ollama use is disabled; poco browser fallbacks remain active")
        ...
        if self.enable_local_ollama and not self.local_ollama_binary and not self._logged_local_missing:
            if os.environ.get("GITHUB_ACTIONS", "").lower() == "true":
                log.info("Local Ollama binary not found inside GitHub Actions container; poco will rely on browser fallbacks until bootstrap succeeds")
            else:
                log.info("Local Ollama binary not found; poco will rely on browser fallbacks until Ollama is installed or OLLAMA_HOST is set")
        ...
        for backend in backend_order:
                    ...
                    try:
                        ...
                        if self._is_local_transport_failure(exc):
                            ...
                            if not self._logged_local_transport_failure:
                                log.info("Local Ollama transport is unavailable for this run; poco will skip further local retries and use fallbacks")
            ...
            for candidate in self._candidate_order(candidates, "remote"):
                try:
                    ...
                    if self._is_remote_transport_failure(exc):
                        self._remote_transport_disabled = True
                        log.info("Remote Ollama transport is unavailable for this run; poco will skip further remote retries and use fallbacks")
                        break
        return ""
