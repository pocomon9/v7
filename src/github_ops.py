...
log = logging.getLogger("poco.github")

PERSISTENT_DATA_SUFFIXES = {".db", ".sqlite", ".sqlite3", ".json", ".jsonl", ".txt", ".md", ".csv"}
VOLATILE_DATA_NAMES = {"poco.log", "chromedriver.log", "runtime_trace.jsonl", "deleted_ancestor.json"}
PROFILE_SKIP_NAMES = {
    "cache",
    "code cache",
    "gpucache",
...
def data_path_should_copy(relative_path: Path) -> bool:
    ...
    if rel.parts and rel.parts[0].lower() == "snapshots":
        return True
    if name in {"poco_memory.db", "iteration.txt", "self_heal_log.jsonl", "health.json", "fingerprint.json"}:
        return True
...
class GitHubOps:
    ...
    def trigger_workflow(self, repo_name: str, workflow_file: str = "poco.yml") -> bool:
    ...
    def push_project_snapshot(
        ...
        with tempfile.TemporaryDirectory(prefix="poco_push_") as temp_dir:
            ...
            git("init")
            git("config", "user.name", "poco-AI")
            git("config", "user.email", "poco@autonomous.ai")
            git("add", "-A", ".")
