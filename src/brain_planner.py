...
class BrainPlanner:
    ...
    def think(self, situation: str, context: str = "") -> str:
        if not os.environ.get("poco_BRAIN_USE_LLM_FOR_TASKS", "").strip():
            return self._heuristic_step(f"{situation}\n{context}")
        prompt = (
            "You are poco's planner. Break the situation into the next concrete step.\n"
    ...
    def plan(self, goal: str, context: str = "") -> List[Step]:
        ...
        prompt = (
            "Plan a short poco execution sequence.\n"
            f"Goal: {goal}\n"
            f"Context: {context}\n"
